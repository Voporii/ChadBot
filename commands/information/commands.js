...
module.exports = {
	...
	name: 'commands',
	description: 'View detalied command information',
	options: [
		{
			name: 'command_name',
			description: 'Command you wish to view',
			type: 3,
			required: false,
		},
	],
...
async function getCMD(client, interaction, input) {
	...
	let info = `❌ Command Not Found \`${input}\``;
	...
	if (cmd.options) {
		...
		info += `\nArguments: \`${formatedoptions.join(' ')}\``;
		embedCmd.setFooter('Syntax: <> = wymagane, [] = opcjonalne');
	}
...
function getAll(client, interaction) {
	...
	const embed = new EmbedBuilder()
		.setAuthor({ name: 'All Bot Commands', iconURL: 'https://cdn.discordapp.com/attachments/825035469711736852/825041736262352986/get-help-48.png'})
